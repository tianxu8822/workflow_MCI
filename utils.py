...
def get_f05(matrix):
    tn, fn, fp, tp = float(matrix[0][0]), float(matrix[0][1]), float(matrix[1][0]), float(matrix[1][1])
    return 1.25 * (tp / (1.25 * tp + fp + 0.25 * fn))


def get_MCC(matrix):
    TP, TN, FP, FN = float(matrix[1][1]), float(matrix[0][0]), float(matrix[1][0]), float(matrix[0][1])
    upper = TP * TN - FP * FN
